/*
 * SQL(Structured Query Language): 구조적 질의 언어. database 제어 언어
...
-- member table에 회원정보를 삽입
-- ORA-00001: 무결성 제약 조건(SCOTT.SYS_C004000)에 위배됩니다
-- 위와 같은 에러가 난다
-- id primary key 제약조건이 설정되어 있고 반드시 유일하게 존재해야 한다.
-- id가 중복되므로 error가 나고 추가가 되지 않는다.
insert into MEMBER(id,password,name,address) values('java','1234','부반장','야탑');

insert into MEMBER(id,password,name,address) values('gandi','1234','김호겸','야탑');

-- name 컬럼에 not null 제약조건이 있어서 반드시 name insert 해야 한다.
insert into MEMBER(id,password,address) values('test','1234','야탑');
-- 가능 address는 별도의 제약조건이 없다.
insert into MEMBER(id,password,name) values('test','1234','황잡스');
-- 전체 회원 정보 조회
select * from member;
-- 회원수 조회
...
-- 아이디가 gandi인 회원의 이름과 주소를 조회
select name,address from member where id='gandi';
-- 아이디가 java이고 패스워드가 1234 인 회원의 이름과 주소 조회
...
-- 수정 결과 확인 id gandi인 member의 name을 조회
...
-- member 정보를 추가 id: jdbc, password: abcd, name:설현,address:부천
...
-- 야탑 address에 사는 회원 이름을 김호겸으로 수정
...
--정렬
select id,password,name,address from member order by id asc;
