...
%>
JSTL로 리스트의 요소의 name을 출력<br>
<%--
 jstl forEach 반복문
 items 속성: 배열 또는 컬렉션
 var 속성: 배열 또는 컬렌션 요소를 순차적으로 저장
 varStatus 속성: index와 count 속성이 있다.
 begin 속성: 몇번째 요소부터 반복문 실행할지 명시
 end 속성: 몇번째 요소까지 반복문 실행할지 명시
 --%>
<c:forEach items="${requestScope.plist }" var="person" varStatus="order">
	${order.count} ${person.name } ${person.money } ${order.index}<br>
</c:forEach>
<hr>
...
<c:forEach begin="${requestScope.startPage }" end="${requestScope.endPage }" var="page">
	<c:choose>
		<c:when test="${page == 8 }"> ${page }</c:when>
