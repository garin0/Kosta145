...
commit
-- 사원 emp 테이블의 사원번호 empno,사원명 ename.직종job,입사일 hiredate
-- 조회
select EMPNO,ENAME,job,hiredate from emp;
--empno 내림차순 정렬하여 조회
select EMPNO,ENAME,job,hiredate from emp order by empno desc;
-- job이 ANALYST인 사원의 사원명과 월급sal 조회
select ename,sal from emp where job='ANALYST';
-- 모든 사원의 사원명,직종,월급,연봉(월급*12)을 조회
-- 월급 내림차순 정렬
select ename,job,sal,sal*12 as income from emp order by sal desc;
-- 직종 job의 종류를 조회
...
-- 월급 sal 1000 이상 2000 이하 사원의 사원명 ename과 
-- job,sal을 sal 내림차순으로 조회
select ename,job,sal from emp where sal>=1000 and sal<=2000 order by sal desc;
-- between a and b
select ename,job,sal from emp where sal between 1000 and 2000 order by sal desc;
-- like : 사원명 ename에 S가 포함된 사원명ename과 job을 조회
select ename,job from emp where ename like '%S%';
-- 커미션comm이 null이 아닌 사원을 조회
select ename,job,comm from emp where comm is not null;
-- date: 사원입사일과 입사 2달 후를 조회
...
-- trunc(): 소수점 이하 버리는 함수
select ename,hiredate,trunc(months_between(sysdate,hiredate)/12) as 연차 from emp;
-- clob:최대 4GB를 지원 
-- (참고)varchar2: 최대 4000byte 지원 
