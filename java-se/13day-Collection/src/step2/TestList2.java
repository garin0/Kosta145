...
public class TestList2 {
	...
	public static void main(String[] args) {
		/*
		 * ArrayList list = new ArrayList(); list.add(new Person("장기혁",30)); //
		 * list의 첫번째 요소의 나이를 출력 // Object type으로 반환되므로 Object casting 해야
		 * getAge()를 호출할 수 있다. Person p = ((Person) list.get(0));
		 * System.out.println(p.getAge()); list.add(new Person("정현지", 26));
		 * list.add(new Person("배서경", 24)); // for문 이용해서 list의 모든 요소의 name을
		 * 출력한다. for(int i = 0 ; i < list.size() ; i++){ Person pl = (Person)
		...
		System.out.println("-------------------");
		// for문을 이용해 30세 이상 Person의 이름 출력
