...
public class ChattingServer {
	/*
	 * 접속한 클라이언트와 통신할 ServerWorker 객체들을 저장한다 접속하면 list에 add 하고 접속해제되면 list에서
	 * remove 한다 public boolean add(E e) public boolean remove(Object o)
	 * 
	 */
	private ArrayList<ServerWorker> list = new ArrayList<ServerWorker>();

	/*
	 * ChattingServer 의 역할은 ServerSocket 생성 후 client 접속시 매번 accept를 실행하여
	 * ServerWorker < Thread 생성 후 start 시켜준다 이 때 생성한 ServerWorker를 list에 add 한다
	 */
	...
	class ServerWorker implements Runnable {
		...
		ServerWorker(Socket socket) {
			...
			System.out.println(clientIp + "님 입장^0^");
			broadcast(clientIp + "님 입장^0^");
		}
		...
		public void run() {
			try {
				...
				list.remove(this); // 스레드 종료전에 리스트에서 삭제
